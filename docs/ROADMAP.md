...
- decisión Dexie/IndexedDB detrás de contratos de repositorio;
- decisión Google Drive opcional detrás de `CloudSyncProvider`, sin Supabase/Firebase/Cloudflare/backend propio;
- primer spike local implementado en PR #14;
- CI propio habilitado;
- pruebas verdes de reapertura, atomicidad, Papelera/restauración, migración e importación idempotente.

Siguiente trabajo dentro del checkpoint:

1. generar y validar proyecto Android con Capacitor;
2. ejecutar el mismo escenario de persistencia en WebView Android real/emulado;
3. probar comprobantes `Blob`, límites, cleanup y fallos de escritura;
4. incorporar `package-lock.json` y usar instalación reproducible en CI;
5. añadir adaptador mínimo Google Drive con OAuth y permisos mínimos;
6. validar dos instalaciones, trabajo offline, reconexión, reintentos e idempotencia;
7. provocar conflicto concurrente y demostrar ausencia de pérdida silenciosa;
...
Materializar APK/AAB reproducible, capacidades nativas y validaciones Android. El CI base ya existe desde Checkpoint 3; aquí se amplía con build Android sin exponer claves.
